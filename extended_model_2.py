...
for sheet in sheet_names:
    ...
    for date_of_interest in data['Date'].unique():
        ...
        # Create interaction terms between AvRating and Tier
        for rating_col in [col for col in group.columns if col.startswith('AvRating_')]:
            for tier_col in [col for col in group.columns if col.startswith('Tier_')]:
                interaction_col = f"{rating_col}_x_{tier_col}"
                group[interaction_col] = group[rating_col] * group[tier_col]
        ...
        group = group.drop(columns=['AvRating_BBB_x_Region_North America', 'AvRating_BBB_x_Tier_SNRFOR'], errors='ignore')
...
output_file = "/Users/lufan/Documents/erasmus_classes/master blok 3/seminar/data_cleaning/extended2_regression_results.xlsx"
