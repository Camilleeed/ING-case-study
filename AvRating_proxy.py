...
for index, row in out_of_sample_data.iterrows(): # iterate every row
    date = row['Date']
    df_today = in_sample_data[in_sample_data['Date'] == date][['Date', 'AvRating']]
    df_yesterday = in_sample_data[in_sample_data['Date'] == date - timedelta(days=1)][['Date', 'AvRating']]
    df_tomorrow = in_sample_data[in_sample_data['Date'] == date + timedelta(days=1)][['Date', 'AvRating']]
