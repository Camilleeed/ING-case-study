...
def apply_ordinal_mappings(df, ordinal_mappings):
    """
    1) Creates new columns (e.g. 'AvRating_ordinal') based on the provided mapping.
    2) For any missing values (NaN) in these new columns, fills with a default:
       - AvRating: 4
       - DocClause: 1
       - Tier: 3
    3) Returns a copy of the modified DataFrame.
    """
    ...
    for col, mapping in ordinal_mappings.items():
        ...
        df[new_col] = df[new_col].fillna(default_values.get(col))
...
for spread in spread_columns:
    ...
    # For unrestricted, change missing values to "BBB" in categorical features
    for col in cat_features_unrestricted:
            if col in df_temp_unres.columns:
                df_temp_unres[col] = df_temp_unres[col].fillna("BBB").astype(str)
...
eval_results_file = os.path.join(output_path, "catboost_missing(imputed)_oos_evaluation.xlsx")
...
predictions_log_file = os.path.join(output_path, "catboost_missing(imputed)_oos_predictions_log.xlsx")
...
predictions_spread_file = os.path.join(output_path, "catboost_missing(imputed)_oos_predictions_spread.xlsx")
